...
function print-fopt { # {{{1
	local s=${1:?} opt=${2:?} o a
	[[ $opt == no-* ]]&& opt="no ${opt#no-}"
	[[ $opt == *=* ]]&& {
		o=${opt%%=*}
		a=${opt#$o=}
		opt="$o $a"
...
function write-fetch-rc { # {{{1
	local usr pwd host port o lmarg
	usr=$1; pwd=$2; host=$3; port=$4; set -- $5
	lmarg='     '
	\cat <<-===
		# ===============================================================
		#   FETCH configuration file generated by $this_pgm
		# ===============================================================
		#   Edit $CFGDIR/accounts
		#   and $CFGDIR/protocols
		#   then run $ARGZERO
		#   to change this file
		# ===============================================================
		...
		poll $host protocol imap
		     ...
		     username '$usr'
		     password '$pwd'
		$( for o; do print-fopt "$lmarg" "$o"; done )
...
function split-into-vars { # {{{1
	IFS=$SQLSEP
	set -- $1
	fI=$1; fE=$2; fP=$3; fH=$4; fR=$5
} # }}}1
function print-one-secret { # {{{1
	print -ru$fdSecrets -- "$fI$TAB$fE:$fP"
} # }}}1
function print-cfg-header { # {{{1
	\cat >&$fdCfg <<-===
		# ===============================================================
		#   SMTPD configuration file generated by $this_pgm
		# ===============================================================
		#   Edit $CFGDIR/accounts
		#   and $CFGDIR/protocols
		#   then run $ARGZERO
		#   to change and install this file
		# ===============================================================
...
function print-one-relay { # {{{1
	local macro dispatch
	macro=${fI}e
	dispatch=${fI}r
	\cat >&$fdCfg <<-===
		$macro="$fE"
		action "$dispatch" relay host "smtps://$fI@$fH:$fR" auth <secrets>
		match from mail-from \$$macro for any action "$dispatch"

		===
} # }}}
function print-cfg-footer { # {{{1
	\cat >&$fdCfg <<-===
...
function open-fd { # {{{1
	...
	nextfd || die "Could not get an fd for ^T$2^t."
	eval "$1=\$REPLY"
	eval "exec $REPLY>$2" || die "Could not open an fd for ^T$2^t."
} # }}}1
function create-smtpd-files { # {{{1
	local tuple fI fE fP fH fR fdSecrets fdCfg REPLY

	needs-path -create -or-warn "$pOut" || return

	open-fd fdSecrets "$pOut/secrets"
	open-fd fdCfg "$pOut/smtpd.conf"

	print-cfg-header

	SQL <<-===SQLite===
		SELECT format('s%02d',id)	AS fI,
			   username				AS fE,
			   password				AS fP,
			   host					AS fH,
			   port					AS fR
...
function install-a-file-work { # {{{1
	local in out file ents mode
	file=$1
	ents=$2
	mode=$3
	...
	needs-file -or-warn "$in"		|| return
	cmp -s "$in" "$out" 			&& return # no complaints
	as-root cp -f "$in" "$out"		|| return
	as-root chown $ents "$out"		|| return
	as-root chmod $mode "$out"		|| return
} # }}}1
function install-a-file { # {{{1
	install-a-file-work "$@" ||
		warn "Did not install ^B$1^b."
} # }}}1
function install-smtpd-files { # {{{1

	makemap $pOut/secrets
	install-a-file secrets.db root:_smtpd 0640

	install-a-file smtpd.conf root:wheel 0644
} # }}}1
function main { # {{{1
	create-db
	$DEBUG && dump-data
	...
	if create-smtpd-files; then
		install-smtpd-files
	else
		warn "Did not create ^Bsmtpd^b configuration files"
	fi
	timestamp >$CFGDIR/.LAST_UPDATED
} # }}}1

needs SQL SQLify needs-file nextfd now as-root
